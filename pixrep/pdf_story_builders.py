...
def build_index_story(gen) -> list:
    ...
    story.append(
        Paragraph(
            f"Code Repository Overview · Generated {datetime.now().strftime('%Y-%m-%d %H:%M')}",
            sub_style,
        )
    )
    ...
    stat_data = [
        [
            ...
            StatBox(
                "SIZE",
                gen._fmt_size(gen.repo.total_size),
                COLORS["green"],
                fonts=gen.fonts,
                width=bw,
                height=50,
            ...
            StatBox(
                "LANGUAGES",
                str(len(gen.repo.language_stats)),
                COLORS["type"],
                fonts=gen.fonts,
                width=bw,
                height=50,
    ...
    for lang, stats in gen.repo.language_stats.items():
        ...
        lang_data.append(
            [
                Paragraph(
                    f'<font color="{COLORS["accent"].hexval()}">{xml_escape(lang)}</font>', ns
                ),
    ...
    lang_table = Table(lang_data, colWidths=[cw * 0.35, cw * 0.2, cw * 0.25, cw * 0.2])
    lang_table.setStyle(
        TableStyle(
            [
    ...
    if len(tree_lines) > 120:
        tree_lines = tree_lines[:120] + [f"  ... ({len(tree_lines)} entries total)"]

    gen._add_code_chunks(
        story,
        tree_lines,
        "text",
        cw,
    ...
    story.append(HeaderBar("File Index", f"{len(gen.repo.files)} files", fonts=gen.fonts, width=cw))
    story.append(Spacer(1, 3 * mm))
        ...
        data.append(
            [
                ...
                Paragraph(
                    ...
                    f"{xml_escape(str(info.path))}</font>",
                    fs,
                ),
                ...
                Paragraph(
                    f'<font color="{COLORS["accent2"].hexval()}">{xml_escape(out_name)}</font>', fs
                ),
            ]
        )
    cols = [cw * 0.06, cw * 0.38, cw * 0.12, cw * 0.12, cw * 0.12, cw * 0.20]
...
def build_file_preamble(gen, file_info: FileInfo):
    ...
    meta = gen._cjk_style("Meta", fontSize=8, textColor=HexColor("#666666"))
    for item in [
    ...
    base_meta_lines = 4
    semantic_meta_lines = (
        1 if (gen.enable_semantic_minimap and file_info.semantic_map.kind != "none") else 0
    )
